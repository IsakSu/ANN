...
def single_layer_perceptron(
    W, X, T, LEARNING_RATE, color_list, training_type, OUT_FOLDER
):
    ...
    if training_type == "delta_training":
        ...
        plot_title = (
            "Perceptron Batch Learning Single Layer Perceptrion with Learning Rate "
        )
    ...
    if training_type == "delta_training":
        ...
        W, num_epochs = sequential_delta_training_until_converge(
            W, X, T, LEARNING_RATE, color_list
        )
        # print("Artists seq", artists)
    elif training_type == "batch_delta_training_convergence":
        W, num_epochs = batch_delta_training_until_convergence(
            W, X, T, LEARNING_RATE, color_list
        )
...
def sequential_delta_training_until_converge(W, X, T, LEARNING_RATE, color_list):
    ...
    while (
        np.where(abs(sum_delta) > convergence_error, True, False).all() or is_first_it
    ):
        is_first_it = False
        ...
        for i in range(input_samples):
            ...
            # Compute gradient
            delta_W = (LEARNING_RATE * (T[i] - np.matmul(W.T, X[:, i])) * X[:, i])[
                :, np.newaxis
            ]
        ...
        print(
            "all(np.where(abs(delta_W) > convergence_error, True, False)) or (is_first_it))",
            all(np.where(abs(delta_W) > convergence_error, True, False))
            or (is_first_it),
        )
